...
#预测
local_data$predict_GLU=round(predict(model_randomForest_NORM_select,local_data),3)
predict_data=round(local_data$predict_GLU,3)
write.table(predict_data,file="data/NORM_select.csv",row.names = FALSE,col.names=FALSE)
